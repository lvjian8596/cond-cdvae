...
def split_dataset(ds, mpds, name, nperpart):
    ...
    theo_mpds = mpds[mpds.theoretical == True][["material_id"]]
    expe_mpds = mpds[mpds.theoretical != True][["material_id"]]
    theo_ds = pd.merge(ds, theo_mpds, on="material_id")
    expe_ds = pd.merge(ds, expe_mpds, on="material_id")
...
"""
python split_dataset.py --ds /home/share/Data/Caly-MP/230617/v2_std_plus_DFTLi/test.feather --nperpart 600
# >>> mpexpe_p{0..5}.txt
awk '{print "--label " $1 " --formula " $2 " --pressure " $3 }' mpexpe_p2.txt|xargs -n 6 python ~/cond-cdvae/scripts/evaluate.py --model_path `pwd` --batch_size 100 --num_batches_to_samples 2 --tasks gen
"""